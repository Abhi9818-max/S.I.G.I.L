      ...
      >
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            {stepIndex > 0 && <Button variant="ghost" size="sm" onClick={handlePrev}><ArrowLeft className="mr-1 h-4 w-4" /> Prev</Button>}
            <Button size="sm" onClick={handleNext}>
              {stepIndex === steps.length - 1 ? 'Finish' : 'Next'}
              {stepIndex < steps.length - 1 ? <ArrowRight className="ml-1 h-4 w-4" /> : <CheckCircle className="ml-1 h-4 w-4" />}
            </Button>
          </div>
        </div>
      </div>
