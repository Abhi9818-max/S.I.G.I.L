...
export default function PublicProfilePage({ initialUserData }: PublicProfilePageProps) {
  ...
  const userAvatar =
    initialUserData.photoURL ||
    `/avatars/avatar${(simpleHash(initialUserData.uid || initialUserData.username) % 12) + 1}.jpeg`;
  const pageTierClass = friendLevelInfo
    ? `page-tier-group-${friendLevelInfo.tierGroup}`
    : 'page-tier-group-1';
